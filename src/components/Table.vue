<template>
  <div class="xfn-table-info">
    <el-card shadow="hover">
      <div class="xfn-table" :style="{background:getTableColor(data.state)}">{{data.tid}}号桌：{{data.state|tableStatus}}</div>
...
export default {
  ...
  methods:{
    getTableColor(state){
      if(state==1) return '#67C23A';
      else if(state==2) return '#E6A23C';
      else if(state==3) return '#F56C6C';
      else return '#909399';
    },
    showTableDetail(){
